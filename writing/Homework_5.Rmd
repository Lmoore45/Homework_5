---
title: "Homework_5"
output: html_document
date: "2024-11-11"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


```{r}
library(tidyverse)
library(ggplot2)
library(lubridate)
```


```{r}
#Read in the data
data <- read_csv("/Users/lmoore/Desktop/Documents/CSU/PhD/Classes/R_Programming_ERHS_535/Homework/Homework 5/Homework_5/Homework_5/data/homicide-data.csv")
```

```{r}
# Change format of date
data <- data %>%
  mutate(reported_date = as.Date(as.character(reported_date), format = "%Y%m%d"))
```



```{r}
# Extract year and month, then count homicides per month
data_monthly <- data %>%
  filter(city == "Baltimore", state == "MD") %>%
  mutate(year = year(reported_date),
         month = month(reported_date, label = TRUE)) %>%
  mutate(
    month_num = month(reported_date),                # Create a numeric month column
    date = make_date(year, month_num, 1)             # Create date using numeric month
  ) %>%
  group_by(year, month, date) %>%                    # Group by year, month, and date for accurate summarization
  summarize(monthly_homicides = n(), .groups = "drop")

```

```{r}

```

